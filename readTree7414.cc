...
*/
    ...
    */
    ...
    for (int sam = 0; sam != nSamples; ++sam) {
        ...
        for (Long64_t it=0; it!=nEntries; ++it) {
            ...
            for (int i=0; i!=_nLeptons; ++i) {
                ...
                if(_flavors[i] == 0){
                  ...
                  if(_isolation[i] < stiso[TMath::Abs(_lEta[i]) > 1.479]){
                ...
                if(_flavors[i] == 1){
                  ...
                  //if(1./_ptratio[i] < (1. / 0.69 + _miniisolation[i][0])){ 
                    if(!(_originReduced[i] == 0 || _isPromptFinalState[i] || _fromHardProcessFinalState[i])){
                      h_secLepIso[0]->Fill(_miniisolation[i][0], scale * _weight);
                      /*
                      if(_originReduced[i] == 1 || _originReduced[i] == 2)
                        leptonProv[0]++;
                      if(_originReduced[i] == 3 || _originReduced[i] == 4)
                        leptonProv[1]++;
                      */
                    }
            ...
            for(int i = 0; i < nLoc; i++)
              std::cout << _lPt[i] << " " << _3dIPsig[i] << " " << _isolation[i] << " " << nLocTight << std::endl;
            */
            ...
            */
            ...
            if(!(_originReduced[leptIndTight[0]] == 0 || _isPromptFinalState[leptIndTight[0]] || _fromHardProcessFinalState[leptIndTight[0]])) continue;
            //if(_originReduced[leptIndLoose] == 3 || _originReduced[leptIndLoose] == 4) continue;
            
            if(_lPt[leptIndLoose] < 10) continue;
            if(_3dIPsig[leptIndLoose] < 4) continue;
            
            if(_flavors[leptIndLoose] == 0){

              if(!_chargeConst[leptIndLoose]) continue;
                
              bool passedMVA = false;
              if (TMath::Abs(_lEta[leptIndLoose]) < 0.8 ) {
                passedMVA = _mvaValue[leptIndLoose] > valuesMVA[0][5];
              } else if (TMath::Abs(_lEta[leptIndLoose]) < 1.479 ) {
                passedMVA = _mvaValue[leptIndLoose] > valuesMVA[1][5];
              } else {
                passedMVA = _mvaValue[leptIndLoose] > valuesMVA[2][5];
              }
            ...
            for (int i=0; i!=_n_Jets; ++i) {
                ...
                for (int j=0; j!=nLocTight; ++j) {
                    //if(TMath::Abs(_3dIPsig[leptIndTight[j]]) > 4) continue;
            ...
            */
            ...
            h_SIP3D[sam]->Fill(_3dIPsig[leptIndLoose]);
            */
            ...
            if(_flavors[leptIndLoose] == 1){
              ...
              //if(1./_ptratio[leptIndLoose] < (1. / 0.69 + _miniisolation[leptIndLoose][0])){
                h_secLepIso[1]->Fill(_miniisolation[leptIndLoose][0], scale * _weight);
                /*
                if(_originReduced[leptIndLoose] == 1 || _originReduced[leptIndLoose] == 2)
                  leptonProv[2]++;
                if(_originReduced[leptIndLoose] == 3 || _originReduced[leptIndLoose] == 4)
                  leptonProv[3]++;
                  */
                ...
                if(_isolation[leptIndLoose] < 0.1){
                  ...
                  if(nLocTight != 2){
                    ...
                    std::cout << "New event" << std::endl;
                    for(int i = 0; i < nLoc; i++)
                      std::cout << _lPt[i] << " " << _3dIPsig[i] << " " << _isolation[i] << " " << nLocTight << " " << (_originReduced[i] == 0 || _isPromptFinalState[i] || _fromHardProcessFinalState[i]) << std::endl;
            
                  }
                  */
                  
                    //std::cout << "what the hell? " << _flavors[leptIndLoose] << " " << _flavors[leptIndTight[0]] << std::endl;
                }
        ...
        //std::cout << "Fake heavy leptons -> (SIP3D < 4): " << leptonProv[0] <<  "; (SIP3D > 4):" << leptonProv[2] << std::endl;
        //std::cout << "Fake light leptons -> (SIP3D < 4): " << leptonProv[1] <<  "; (SIP3D > 4):" << leptonProv[3] << std::endl;
    ...
    TFile *file_ele = TFile::Open("fakerate_ele_noCorr_sip3dmore4.root","RECREATE");
    ...
    //c1->cd(2);
    TFile *file_mu = TFile::Open("fakerate_mu_noCorr_sip3dmore4.root","RECREATE");
    ...
    TFile *file_lpt = TFile::Open("lpt_sip3dmore4.root","RECREATE");
    ...
    TFile *file_closestjetpt = TFile::Open("closestjetpt_sip3dmore4.root","RECREATE");
