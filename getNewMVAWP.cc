...
void showHist(TVirtualPad *, TH1D *, TH1D *, string, string, string, double, TLegend*);
...
*/
    ...
    //TString fileList[nSamples] = {"data_combine.root"};
    ...
    */
    ...
    Color_t colsStack[nSamples] = {kGray, kGray, kBlack 
    };
    ...
    TString names[nSamples] = {"singleTop", "singleTopBar", "ttbar_2l"};
    ...
    TH1D* h_pt_light_heavy[2][2][4];
    TH1D* h_pt_light_heavy_scan[200][2][4];
    TGraphAsymmErrors* g_pt_light_heavy[2][3];

    const int ptnbin = 9;
    double ptbins[ptnbin] = {10., 20., 30., 40., 50., 70., 100., 150., 200.};
    ...
    for(int i = 0; i < 2; i++){
      ...
      for(int j = 0; j < 2; j++){
        for(int k = 0; k < 4; k++){
          ...
          name = Form("h_pt_light_heavy_%d_%d_%d",i,j,k);
          h_pt_light_heavy[i][j][k] = new TH1D(name, "pt_light_heavy;p_{T}; FR", ptnbin-1, ptbins);
          //h_pt_eta_2D[i][j]->SetLineColor(colsStack[i]);
          //h_pt_eta_2D[i][j]->SetFillColor(colsStack[i]);
          h_pt_light_heavy[i][j][k]->SetFillStyle(1001);
          //h_pt_eta_2D[i][j]->SetMarkerColor(colsStack[i]);
          h_pt_light_heavy[i][j][k]->Sumw2();
        }
      }
    }
    */
    ...
    for(int j = 0; j < 2; j++){
      ...
      for(int k = 0; k < 4; k++){
        for(int scan = 0; scan < 200; scan++){
          ...
          name = Form("h_pt_light_heavy_scan_%d_%d_%d",j,k,scan);
          h_pt_light_heavy_scan[scan][j][k] = new TH1D(name, "pt_light_heavy_scan;p_{T}; FR", ptnbin-1, ptbins);
          h_pt_light_heavy_scan[scan][j][k]->SetFillStyle(1001);
          h_pt_light_heavy_scan[scan][j][k]->Sumw2();
    ...
    for (int sam = 0; sam != nSamples; ++sam) {
        ...
        int lightFakeLepton = 0;
        int heavyFakeLepton = 0;
        ...
        for (Long64_t it=0; it!=nEntries; ++it) {
            ...
            int promptLeptonID = -1;
            int fakeLeptonID = -1;

            //std::cout << "New event" << std::endl;

            for(int num = 0; num < nLoc; num++){
              if(_originReduced[leptInd[num]] == 0 || _isPromptFinalState[leptInd[num]] || _fromHardProcessFinalState[leptInd[num]]){
              //if(_originReduced[leptInd[num]] == 0){
                promptLepton++;
                promptLeptonID = leptInd[num];
                //std::cout << "Prompt MVA: " << _mvaValue[promptLeptonID] << std::endl;
              }
              if(!(_originReduced[leptInd[num]] == 0 || _isPromptFinalState[leptInd[num]] || _fromHardProcessFinalState[leptInd[num]])){
              //if(_originReduced[leptInd[num]] != 0){
                fakeLepton++;
                fakeLeptonID = leptInd[num];
                //std::cout << "Fake MVA: " << _mvaValue[fakeLeptonID] << std::endl;
            ...
            //std::cout << _chargeConst[fakeLeptonID] << " " << fakeLeptonID << " " << _lPt[fakeLeptonID] << " " << TMath::Abs(_3dIPsig[fakeLeptonID]) << std::endl;
            //continue;
            ...
            if(_flavors[fakeLeptonID] != 0) continue;
            ...
            //std::cout << fakeLeptonID << " " << _mvaValue[fakeLeptonID] << " " << promptLeptonID << " " << _mvaValue[promptLeptonID] << std::endl;
            
            
            

            /*
            std::cout << promptLepton << " " << fakeLepton << " " << nLocTight << std::endl;
            continue;
            */

            /*
            if(_lPt[fakeLeptonID] < 10) continue;
            if(!_chargeConst[fakeLeptonID]) continue;
            if(TMath::Abs(_3dIPsig[fakeLeptonID]) > 4) continue;
            */
            ...
            */
            ...
            //----------------------------------------------
            // Lepton pt correction, instead of lepton pt, mother of lepton pt is used

            double leptPtCorr = 0.;
            
            if(_ptrel[fakeLeptonID] > 7.2){
              leptPtCorr = _lPt[fakeLeptonID] * (1 + TMath::Max(0., _miniisolation[fakeLeptonID][0] - 0.16));
            }
            else{
              leptPtCorr = TMath::Max(_lPt[fakeLeptonID], _closeJetPtAll[fakeLeptonID] * 0.76);
            }
            /*
            std::cout << "New Event" << std::endl;
            std::cout << _flavors[promptLeptonID] << " " << _lPt[promptLeptonID] << " " << _lEta[promptLeptonID] << " " << _mvaValue[promptLeptonID] << " " << _isolation[promptLeptonID] << std::endl;
            std::cout << _flavors[fakeLeptonID] << " " << _lPt[fakeLeptonID] << " " << leptPtCorr << " " << TMath::Abs(_3dIPsig[fakeLeptonID]) << " " << _lEta[fakeLeptonID] << " " << _mvaValue[fakeLeptonID] << " " << _isolation[fakeLeptonID] << " " << _ptrel[fakeLeptonID] << std::endl;
            */
            //-------------------------------------

            // Code for obtaining FR raion in light and heavy quarks
            /*
            if(_originReduced[fakeLeptonID] == 1 || _originReduced[fakeLeptonID] == 2){
              
              bool passedMVA = false;
              int etaIndex = -1;
              if (TMath::Abs(_lEta[fakeLeptonID]) < 0.8 ) {
                passedMVA = _mvaValue[fakeLeptonID]> valuesMVA[0][3];
                etaIndex = 1;
              } 
              else if (TMath::Abs(_lEta[fakeLeptonID]) < 1.479 ) {
                passedMVA = _mvaValue[fakeLeptonID]> valuesMVA[1][3];
                etaIndex = 2;
              } 
              else {
                passedMVA = _mvaValue[fakeLeptonID]> valuesMVA[2][3];
                etaIndex = 3;
              }

              
              
              if(passedMVA && _isolation[fakeLeptonID] < stiso[TMath::Abs(_lEta[fakeLeptonID]) > 1.479]){
              //if(_isolation[fakeLeptonID] < 0.1){
                h_pt_light_heavy[1][0][0]->Fill(leptPtCorr);
                h_pt_light_heavy[1][0][etaIndex]->Fill(leptPtCorr);
                
              }
              else{
                h_pt_light_heavy[0][0][0]->Fill(leptPtCorr);
                h_pt_light_heavy[0][0][etaIndex]->Fill(leptPtCorr);
              }
              ...
              if(passedMVA && _isolation[fakeLeptonID] < stiso[TMath::Abs(_lEta[fakeLeptonID]) > 1.479]){
              //if(_isolation[fakeLeptonID] < 0.1){
                h_pt_light_heavy[1][1][0]->Fill(leptPtCorr);
                h_pt_light_heavy[1][1][etaIndex]->Fill(leptPtCorr);
                
              }
              else{
                h_pt_light_heavy[0][1][0]->Fill(leptPtCorr);
                h_pt_light_heavy[0][1][etaIndex]->Fill(leptPtCorr);
              }
            ...
            */
            ...
            if (TMath::Abs(_lEta[fakeLeptonID]) > 1.479) continue;

            // Code for MVA WP scan
            ...
            if(_originReduced[fakeLeptonID] == 1 || _originReduced[fakeLeptonID] == 2){
              ...
              if(passedMVA && _isolation[fakeLeptonID] < stiso[TMath::Abs(_lEta[fakeLeptonID]) > 1.479]){
              //if(_isolation[fakeLeptonID] < 0.1){
                h_pt_light_heavy_scan[0][0][0]->Fill(leptPtCorr);
                h_pt_light_heavy_scan[0][0][etaIndex]->Fill(leptPtCorr);
                
              }
              else{
                
                for(int i = 1; i < 200; i++){
                  bool passedMVAscan = false;
                  double MVAvalueScan = -1.01 + 0.01*i;
                  if (TMath::Abs(_lEta[fakeLeptonID]) < 0.8 ) {
                    passedMVAscan = _mvaValue[fakeLeptonID] > MVAvalueScan;
                  } 
                  else if (TMath::Abs(_lEta[fakeLeptonID]) < 1.479 ) {
                    passedMVAscan = _mvaValue[fakeLeptonID] > MVAvalueScan;
                  } 
                  ...
                  if(passedMVAscan){
                    h_pt_light_heavy_scan[i][0][0]->Fill(leptPtCorr);
                    h_pt_light_heavy_scan[i][0][etaIndex]->Fill(leptPtCorr);
                  }
                }
            ...
            // light flavors
              ...
              if(passedMVA && _isolation[fakeLeptonID] < stiso[TMath::Abs(_lEta[fakeLeptonID]) > 1.479]){
              //if(_isolation[fakeLeptonID] < 0.1){
                h_pt_light_heavy_scan[0][1][0]->Fill(leptPtCorr);
                h_pt_light_heavy_scan[0][1][etaIndex]->Fill(leptPtCorr);
                ...
                for(int i = 1; i < 200; i++){
                  ...
                  if(passedMVAscan){
                    h_pt_light_heavy_scan[i][1][0]->Fill(leptPtCorr);
                    h_pt_light_heavy_scan[i][1][etaIndex]->Fill(leptPtCorr);
                  }
                }
        ...
        std::cout << "Light Fake lepton: " << lightFakeLepton << std::endl;
        std::cout << "Heavy Fake lepton: " << heavyFakeLepton << std::endl;
    ...
    for(int i = 0; i < 4; i++){
      for(int j = 0; j < 2; j++)
        //g_pt_light_heavy[j][i] = new TGraphAsymmErrors(h_pt_light_heavy[1][j][i], h_pt_light_heavy[0][j][i]);
        //h_pt_light_heavy[1][j][i]->Divide(h_pt_light_heavy[0][j][i]);
        for(int scan = 1; scan < 200; scan++){
          TH1D * hcopy = (TH1D*)h_pt_light_heavy_scan[0][j][i]->Clone("hcopy");
          hcopy->Divide(h_pt_light_heavy_scan[scan][j][i]);
          h_pt_light_heavy_scan[scan][j][i]=hcopy;
          //h_pt_light_heavy_scan[0][j][i]->Divide(h_pt_light_heavy_scan[scan][j][i]);
        }
    }
    
    /*
    TLegend *leg = new TLegend(0.5, 0.6, 0.8, 0.8);
    leg->AddEntry(h_pt_light_heavy[1][0][0], "light", "l");
    leg->AddEntry(h_pt_light_heavy[1][1][0], "heavy", "l");
    leg->SetBorderSize(0);
    ...
    showHist(c1->cd(1), h_pt_light_heavy[1][0][0], h_pt_light_heavy[1][1][0], "", "p_{T}^{corr}", "FR", 1., leg);
    showHist(c1->cd(2), h_pt_light_heavy[1][0][1], h_pt_light_heavy[1][1][1], "", "p_{T}^{corr}", "FR", 1., leg);
    showHist(c1->cd(3), h_pt_light_heavy[1][0][2], h_pt_light_heavy[1][1][2], "", "p_{T}^{corr}", "FR", 1., leg);
    showHist(c1->cd(4), h_pt_light_heavy[1][0][3], h_pt_light_heavy[1][1][3], "", "p_{T}^{corr}", "FR", 1., leg);
    */
    
    int j = 180;

    TLegend *leg = new TLegend(0.5, 0.6, 0.8, 0.8);
    leg->AddEntry(h_pt_light_heavy_scan[j][1][0], "light", "l");
    leg->AddEntry(h_pt_light_heavy_scan[j][0][0], "heavy", "l");
    leg->SetBorderSize(0);
    ...
    //for(int i = 1; i < 200; i++){
      ...
      std::cout << "***********************************************************" << std::endl;
      std::cout << "New MVA WP: " << -1.01 + 0.01 * i << std::endl; 
      showHist(c1->cd(1), h_pt_light_heavy_scan[i][1][0], h_pt_light_heavy_scan[i][0][0], "", "p_{T}^{corr}", "FR", 1., leg);
      showHist(c1->cd(2), h_pt_light_heavy_scan[i][1][1], h_pt_light_heavy_scan[i][0][1], "", "p_{T}^{corr}", "FR", 1., leg);
      showHist(c1->cd(3), h_pt_light_heavy_scan[i][1][2], h_pt_light_heavy_scan[i][0][2], "", "p_{T}^{corr}", "FR", 1., leg);
      showHist(c1->cd(4), h_pt_light_heavy_scan[i][1][3], h_pt_light_heavy_scan[i][0][3], "", "p_{T}^{corr}", "FR", 1., leg);
...
void showHist(TVirtualPad* c1, TH1D *hist, TH1D *hist2, string title, string titleX, string titleY, double num, TLegend *leg){   
    ...
    */
    ...
    extraText.DrawLatex(0.54, 0.93,"Preliminary");
    ...
    histcopy->GetYaxis()->SetTitle("light/heavy");
